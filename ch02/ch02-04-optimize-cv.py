...
# -----------------------------------
# out-of-fold에서의 임계값(threshold)의 최적화
# -----------------------------------
...
# 실제값과 예측값이 train_y, train_pred_prob으로 함.
...
for i, (tr_idx, va_idx) in enumerate(kf.split(train_pred_prob)):
    ...
    # 학습 데이터로 임계값(threshold)를 실시하여 검증 데이터로 평가를 수행
...
# 각 fold 임계값(threshold) 평균을 테스트 데이터에 적용
